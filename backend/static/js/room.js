...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // 打开用户设置模态框
    document.getElementById('user-setup-modal').classList.add('show');
    ...
    // 使用common.js中的函数处理用户设置确认
        ...
        // 如果是首次设置或者修改了用户名，才进行连接或更新
        if (socket === null) {
            // 首次设置：连接SocketIO进入房间
            username = newUsername;
            connectSocket();
        } else if (user_id && newUsername !== originalUsername) {
            // 已连接但修改了用户名：更新用户名
            username = newUsername;
            originalUsername = newUsername;

            // 使用现有的set_userinfo事件更新用户名
            socket.emit(ClientMessageType.SetUserInfo, { [ClientDataKey.Username]: newUsername });
        }
    ...
    if (typeof setupBasicSocketListeners === 'function' && socket) {
        setupBasicSocketListeners(socket, {
            ...
            onUsernameError: function(data) {
                ...
                // 显示错误消息
                const usernameError = document.getElementById('username-error');
                usernameError.textContent = data.error || data.message || '用户名错误';
                usernameError.style.display = 'block';
            },
            customConnectedHandler: function(data) {
                console.log('收到服务器连接确认:', data);
...
function updateRoomInfo(data) {
    ...
    if (!document.getElementById('change-avatar-btn')) {
        ...
        btn.id = 'change-avatar-btn';
        btn.className = 'btn btn-info mb-2';
        btn.textContent = '设置玩家信息';
        btn.addEventListener('click', function () {
            document.getElementById('user-setup-modal').classList.add('show');
        });
        controlsContent.appendChild(btn);
    }
...
function generatePresetAvatars() {
    ...
    presetAvatars.forEach((avatarUrl, index) => {
        ...
        div.addEventListener('click', function () {
            ...
            // 调用setAvatar更新用户头像
            // setAvatar函数内部已包含保存用户信息到localStorage的逻辑
...
function setAvatar(avatarUrl) {
    if (socket && user_id) {
        // 使用common.js中的函数处理头像URL
        const cleanAvatarUrl = typeof getCleanAvatarUrl === 'function' ? getCleanAvatarUrl(avatarUrl) : avatarUrl;
        socket.emit(ClientMessageType.SetAvatar, { [ClientDataKey.AvatarURL]: cleanAvatarUrl });
